/**
 * THE SIMULATION — 3D Experience Module
...
(function () {
  ...
  function buildScene(engine, level, shell) {
    ...
    var core = engine.addBox(0, 0.5, 0, 1.2, 1.0, 1.2, parseInt('0x080818', 16));
  ...
  var config = {
    ...
    1: {
      ...
      tasks: [{
        ...
        output: '[OK] DETECT RECON executed — Detect scanning before weaponization',
        onSuccess: function (shell) { shell.score += 100; shell.updateScore(); }
      }],
      branch: {
        title: 'Decision point — Level 1',
        desc: 'Your Cyber attack kill chain approach affects the next phase.',
        options: [
    ...
    2: {
      ...
      tasks: [{
        ...
        errorType: 'wrong_command',
        validate: function () { return true; },
        output: '[OK] BLOCK DELIVERY executed — Identify phishing email among noise',
        onSuccess: function (shell) { shell.score += 100; shell.updateScore(); }
      }],
      branch: {
    ...
    3: {
      ...
      tasks: [{
        ...
        errorType: 'wrong_command',
        validate: function () { return true; },
        output: '[OK] PATCH EXPLOIT executed — Patch or monitor vulnerable host',
        onSuccess: function (shell) { shell.score += 100; shell.updateScore(); }
      }],
      branch: {
    ...
    4: {
      ...
      tasks: [{
        ...
        errorType: 'wrong_command',
        validate: function () { return true; },
        output: '[OK] KILL C2 executed — Detect command channel',
        onSuccess: function (shell) { shell.score += 100; shell.updateScore(); }
      }]
    }
  ...
  var PLAYBOOK_TITLE = 'THE SIMULATION — operator runbook';
  var CORE_CONCEPT = 'Cyber attack kill chain';
  var LEVEL_BRIEFINGS = {
    1: {
