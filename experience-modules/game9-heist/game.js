...
(function () {
  ...
  function buildScene(engine, level, shell) {
    ...
    var core = engine.addBox(0, 0.5, 0, 1.2, 1.0, 1.2, parseInt('0x0a0d1a', 16));
  ...
  var config = {
    ...
    1: {
      ...
      tasks: [{
        ...
        output: '[OK] RECON executed — Map checkpoints without rapid-requests trigger',
        onSuccess: function (shell) { shell.score += 100; shell.updateScore(); }
      }],
      branch: {
        title: 'Decision point — Level 1',
        desc: 'Your Red team attack planning approach affects the next phase.',
        options: [
    ...
    2: {
      ...
      tasks: [{
        ...
        errorType: 'wrong_command',
        validate: function () { return true; },
        output: '[OK] STAGE TOOL executed — Obtain admin/tool without brute-force',
        onSuccess: function (shell) { shell.score += 100; shell.updateScore(); }
      }],
      branch: {
    ...
    3: {
      ...
      tasks: [{
        ...
        errorType: 'wrong_command',
        validate: function () { return true; },
        output: '[OK] COPY DATA executed — Copy nodes under guard vision limit',
        onSuccess: function (shell) { shell.score += 100; shell.updateScore(); }
      }],
      branch: {
    ...
    4: {
      ...
      tasks: [{
        ...
        errorType: 'wrong_command',
        validate: function () { return true; },
        output: '[OK] SUPPRESS ALERT executed — Suppress escalation during alarm',
        onSuccess: function (shell) { shell.score += 100; shell.updateScore(); }
      }]
    }
  ...
  var PLAYBOOK_TITLE = 'THE HEIST — operator runbook';
  var CORE_CONCEPT = 'Red team attack planning';
  var LEVEL_BRIEFINGS = {
    1: {
