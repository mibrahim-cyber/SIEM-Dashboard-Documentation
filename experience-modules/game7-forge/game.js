/**
 * THE FORGE — 3D Experience Module
...
(function () {
  ...
  function buildScene(engine, level, shell) {
    ...
    var core = engine.addBox(0, 0.5, 0, 1.2, 1.0, 1.2, parseInt('0x030a04', 16));
  ...
  var config = {
    ...
    1: {
      ...
      tasks: [{
        ...
        output: '[OK] ADD TRIGGER executed — Assemble trigger + filter from template',
        onSuccess: function (shell) { shell.score += 100; shell.updateScore(); }
      }],
      branch: {
        title: 'Decision point — Level 1',
        desc: 'Your Detection rule construction approach affects the next phase.',
        options: [
    ...
    2: {
      ...
      tasks: [{
        ...
        errorType: 'wrong_command',
        validate: function () { return true; },
        output: '[OK] TUNE THRESH executed — Reduce noise below 10% FP rate',
        onSuccess: function (shell) { shell.score += 100; shell.updateScore(); }
      }],
      branch: {
    ...
    3: {
      ...
      tasks: [{
        ...
        errorType: 'wrong_command',
        validate: function () { return true; },
        output: '[OK] ADD FILTER executed — Link two events into one rule',
        onSuccess: function (shell) { shell.score += 100; shell.updateScore(); }
      }],
      branch: {
    ...
    4: {
      ...
      tasks: [{
        ...
        errorType: 'wrong_command',
        validate: function () { return true; },
        output: '[OK] TEST RULE executed — Rule must catch sample malicious event',
        onSuccess: function (shell) { shell.score += 100; shell.updateScore(); }
      }]
    }
  ...
  var PLAYBOOK_TITLE = 'THE FORGE — operator runbook';
  var CORE_CONCEPT = 'Detection rule construction';
  var LEVEL_BRIEFINGS = {
    1: {
