/**
 * DEEP ARCHIVE — 3D Experience Module
...
(function () {
  ...
  function buildScene(engine, level, shell) {
    ...
    var core = engine.addBox(0, 0.5, 0, 1.2, 1.0, 1.2, parseInt('0x06040c', 16));
  ...
  var config = {
    ...
    1: {
      ...
      tasks: [{
        ...
        output: '[OK] SORT LOGS executed — Order entries by timestamp',
        onSuccess: function (shell) { shell.score += 100; shell.updateScore(); }
      }],
      branch: {
        title: 'Decision point — Level 1',
        desc: 'Your Log forensics and timeline reconstruction approach affects the next phase.',
        options: [
    ...
    2: {
      ...
      tasks: [{
        ...
        errorType: 'wrong_command',
        validate: function () { return true; },
        output: '[OK] ANCHOR TIME executed — Place first 5 events on timeline',
        onSuccess: function (shell) { shell.score += 100; shell.updateScore(); }
      }],
      branch: {
    ...
    3: {
      ...
      tasks: [{
        ...
        errorType: 'wrong_command',
        validate: function () { return true; },
        output: '[OK] LINK EVENT executed — Connect cause/effect between entries',
        onSuccess: function (shell) { shell.score += 100; shell.updateScore(); }
      }],
      branch: {
    ...
    4: {
      ...
      tasks: [{
        ...
        errorType: 'wrong_command',
        validate: function () { return true; },
        output: '[OK] FIND ROOT executed — Identify initiating event',
        onSuccess: function (shell) { shell.score += 100; shell.updateScore(); }
      }]
    }
  ...
  var PLAYBOOK_TITLE = 'DEEP ARCHIVE — operator runbook';
  var CORE_CONCEPT = 'Log forensics and timeline reconstruction';
  var LEVEL_BRIEFINGS = {
    1: {
