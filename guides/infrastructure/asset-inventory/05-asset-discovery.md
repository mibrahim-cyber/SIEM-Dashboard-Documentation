...
The built-in sample logs target known asset IPs by design; this ensures training exercises produce correlated results and should not be read as a statement of registry completeness compared to real passive-only discovery. Search assets... cannot find what is not registered. CVE filter finds vulnerable known assets only, not vulnerable unknown hosts. Blocked IP count tracks asset IPs in block list, not discovered attackers.
