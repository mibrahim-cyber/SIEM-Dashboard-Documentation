...
No. It injects synthetic logs into the client-side processing pipeline and SQLite alert store only. No packets leave the browser to external targets. SOAR may call AbuseIPDB for external IPs in simulated alerts if keys are configured; be aware of API quota in demos.

#### Why do some simulated logs not generate alerts?
...
Double-triggering is blocked by `campaignRunning` guard. Campaign logs marked `_simulated` may still trigger real AbuseIPDB lookups and watchlist adds for high scores; disable keys or use internal IP ranges in custom tests if undesired. Eight-second window means impatient clickers may think it failed if first batch is slow. Simulation does not reset `logsProcessed`; **LOGS** still climbs, which is correct behaviour.
