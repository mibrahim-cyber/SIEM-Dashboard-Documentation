...
> **Technical note:** `correlateAlerts` assigns `id: inc-${alert.id}` using the seed alert's ID; sorting uses `lastSeen` descending. Category-based cross-IP clustering (`CATEGORY_WINDOW_MS = 120_000`) exists in comments but is not implemented in the shipped loop, only IP clustering runs.
