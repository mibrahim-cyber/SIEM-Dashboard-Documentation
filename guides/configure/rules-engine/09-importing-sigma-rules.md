...
tags: attack.tXXXX
```
HABIBI detection logic uses imperative JavaScript on normalised log objects from ingestion pipeline, field names like `sourceIp`, `eventType`, nested `event.outcome`, `http.request.body`. Mapping Sigma `Image|endswith` to log schema requires field mapping layer absent in demo. `Correlation Builder` condition fields (`source.ip`, `event.outcome`, etc.) gesture toward ECS-like mapping but do not import Sigma either.

> **Technical note:** Community Sigma repository rules number thousands, automated import needs Sigma compiler (e.g., sigmac) targeting HABIBI log schema; not implemented.
