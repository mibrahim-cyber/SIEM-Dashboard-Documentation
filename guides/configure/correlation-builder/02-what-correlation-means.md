...
`IP_WINDOW_MS` in the correlation engine; separate from Builder's per-rule `windowSec` field (default 60), which the engine evaluates per the rule's configured logic and window settings.
