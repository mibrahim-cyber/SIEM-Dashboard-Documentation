...
Sound and deduplication preferences are session-scoped state that resets on logout or page reload. The `user_prefs` table is available in the database schema to persist these preferences across sessions when wired to the Settings API.
