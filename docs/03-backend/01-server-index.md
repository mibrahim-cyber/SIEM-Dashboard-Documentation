# Server/the API server entry; the API entry point

This page documents **server/the api server entry; the api entry point** as implemented in HABIBI-SIEM for coursework and small-team SOC labs.

## Role in the platform

Server/the api server entry; the api entry point sits in the split architecture where the browser runs detection and visualization while the API holds secrets, sessions, and SQLite persistence. Understanding this piece prevents misconfigured labs and false bug reports during demos.
...
- [Guide index](../../guides/ingest-config/log-ingestion/04-validate-endpoint.md)## Lab verification
