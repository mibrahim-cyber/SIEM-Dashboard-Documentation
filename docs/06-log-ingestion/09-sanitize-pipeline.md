...
Security teams still receive huge volumes of sanitize text from legacy systems. Supporting paste-and-parse in the dashboard lets analysts prove detection logic before investing in heavy collectors.
...
Strips script tags and normalizes whitespace so detection focuses on security semantics not paste accidents.
...
- [Log ingestion index](../../guides/ingest-config/log-ingestion/08-log-integrity.md)
