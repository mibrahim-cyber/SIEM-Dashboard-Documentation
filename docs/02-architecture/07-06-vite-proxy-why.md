...
Why browser calls /api on same origin in labs sits in the split architecture where the browser runs detection and visualization while the API holds secrets, sessions, and SQLite persistence. Understanding this piece prevents misconfigured labs and false bug reports during demos.
